...
export class MessageHandler {
  ...
  async handle(message) {
    ...
    if (
      ...
      !this.config.allowGroupMenu
    ) {
      return;
    }

    const member =
      await this.#memberForMessage(
        message,
        actorJid
      );

    return this.whatsapp.sendMenu(
      remoteJid,
      this.menu.main(member),
      message
    );
  }

  async #resolveActorJid(
    message,
    remoteJid
  ) {
    const group =
    ...
    for (
      ...
      const resolved =
        await this.whatsapp.resolveJid(
          candidate
        );
  ...
  async #menuAction(
      ...
      return send(
        member
          ? {
              ...
              footer:
                this.settings().botFooter
            }
          : {
              kind: 'text',
              text:
                'I cannot find a completed membership record for this WhatsApp number.'
            }
      );
    }
    ...
    if (
      ...
      await this.members.setReminders(
        member.id,
        enabled
      );
      ...
      return send(
        member
          ...
          : {
              kind: 'text',
              text:
                'No completed membership record was found.'
            }
      );
    }
    ...
    if (id === 'member:delete_no') {
      return send({
        kind: 'text',
        text:
          '✅ No changes were made. Your membership remains active.'
      });
    }
    ...
    if (id === 'menu:close') {
      return send({
        kind: 'text',
        text:
          `Menu closed. Send ${this.settings().keyword} whenever you need it again.`
      });
    }
    ...
    if (id === 'help') {
      return send({
        kind: 'text',
        text:
          `Send ${this.settings().keyword} to open the member menu at any time.`
      });
    }
  ...
  async #sendResult(
    ...
    return this.whatsapp.sendToJid(
      destination,
      payload,
      {
        quoted
