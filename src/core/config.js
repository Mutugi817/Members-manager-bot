...
export function loadConfig() {
  ...
  const membersGroupNotifications =
    bool(
      process.env
        .MEMBERS_GROUP_NOTIFICATIONS,
      true
    );
  ...
  const config = {
    ...
    /*
     * Automatic members-group notification.
     */
    membersGroupJid,
    ...
    welcomeTitle:
      process.env.WELCOME_TITLE ||
      'Grace Encounter Members',

    welcomeIntro:
      process.env.WELCOME_INTRO ||
      'Welcome. We are glad to serve you.',

    menuTitle:
      process.env.MENU_TITLE ||
      'Member Menu',

    menuIntro:
      process.env.MENU_INTRO ||
      'Please choose a service below.',

    allowMemberCommands:
      bool(
        process.env
          .ALLOW_MEMBER_COMMANDS,
        true
      ),
  ...
  if (
    config.membersGroupNotifications &&
    !config.membersGroupJid
  ) {
    throw new Error(
  ...
  if (
    config.membersGroupJid &&
    !config.membersGroupJid.endsWith(
      '@g.us'
    )
  ) {
    throw new Error(
